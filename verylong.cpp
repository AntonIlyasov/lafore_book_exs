...
{
  char temp[SZ];          //создать копию
  strcpy(temp,vlstr);
  // cout << strrev(temp);   //перевернуть копию и вывести ее
}
...
//сложение
{
  ...
  //установить перенос в 1
  carry = 0;
  //иначе перенос = 0
  temp[j] = digitsum+'0'; //вставить символ в строку
  }
  if(carry==1)
  //если перенос в конце,
  temp[j++] = '1';
  //последняя цифра = 1
  temp[j] = '\0';
  //поставить ограничитель строки
  return verylong(temp);
  //вернуть временный verylong
}
...
verylong verylong::operator * (const verylong v)//умножение
{
  ...
  return tempsum;
  //вернуть полученную текущую сумму
}
...
verylong verylong::multdigit(const int d2) const
{
  ...
  // в этом сверхбольшом
  int d1 = vlstr[j]-'0';
  //получить значение разряда
  int digitprod = d1 * d2; //умножить на цифру
  digitprod += carry;
  //добавить старый перенос
  if( digitprod >= 10 )
  //если возник новый перенос,
  {
  ...
  if(carry != 0)
  //если на конце перенос,
  temp[j++] = carry+'0';
  //это последний разряд
  temp[j] = '\0';
  //поставить ограничитель
  return verylong(temp);
  //вернуть сверхбольшое число
}
